...
// Syllables and exclusion words used for English.
var englishSyllables = {
	...
	addSyllables: [ "ia", "riet", "dien", "iu", "io", "ii", "[aeiouym][bdp]l", "[aeiou]{3}", "^mc", "ism[\s/.]", "([^aeiouy])\1l[\s/.]", "[^l]lien", "^coa[dglx].", "[^gq]ua[^auieo]", "dnt[\s/.]", "uity[\s/.]", "ie(r|st)", "[aeiouy]ing", "[aeiouw]y[aeiou]" ],
	exclusionWords: [
...
// Syllables and exclusion words used for Dutch.
var dutchSyllables = {
	subtractSyllables: [ "ue+$", "ue$", "dge+$", "dge$","[tcp]iënt" ,"ace$", "ace+$", "[br]each", "[ainpr]tiaal", "[io]tiaan", "gua[yc]", "[^i]deal", "^deal", "tive$", "base[^enr]", "base$", "load", "close[^rt]", "close$", "[^e]coke", "^coke", "drive[^r]", "drive$", "[^s]core$", "^core$", "face[^nt]",
		"face$", "cache[^clntx]", "cache$", "game[^nlr]", "game$" ],
	addSyllables: [ "aä", "aeu", "aie", "ao", "ë", "eo", "eú", "ieau", "ea$", "ea+$", "ea[^u]", "ei[ej]", "eu[iu]", "ï", "iei", "ienne", "[^l]ieu[^w]", "[^l]ieu$", "[^l]ieu+$",  "i[auiy]", "stion", "[^cstx]io", "^sion","riè", "oö", "oa", "oeing", "oeyo", "oie", "[eu]ü", "[^q]u[oaeè]", "uie", "[aeolu]y[eéèaoóu]",
		"[bhnpr]ieel", "[bhnpr]iël" ],

	// Exclusion words that should be found at the beginning or end of a string.
	exclusionCompounds:[
		{ word: "byte", syllabes: 1 },
	...
	// Exclusion words that should be found at the end of a string.	
	exclusionCompoundEnds: [
		{ word: "tea", syllables: 1 },
		{ word: "time", syllables: 1 },
		{ word: "force", syllables: 1 }
	],

	// Exclusion words that should be found anywhere in a string.
	exclusionWordParts: [
	...
	// Exclusion words that should only be found as independent words.
	exclusionWords: [
 ...
 * Exclusionwords are removed from the text to be counted separately.
 *
 * @returns {object}
 */
module.exports = function( locale ) {
	if ( ! isUndefined( locale ) ) {
		switch (true) {
			case locale.indexOf("en_") > -1:
				return englishSyllables;
				break;
			case locale.indexOf("nl_") > -1:
				return dutchSyllables;
				break;
			default:
				break;
		}
